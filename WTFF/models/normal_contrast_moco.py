...
class MoCo_Model(nn.Module):
    def __init__(self, args, queue_size=65536, momentum=0.999, temperature=0.07):
        ...
        # Load model
        self.encoder_q = getattr(models, args.backbone)()  # Query Encoder

        self.encoder_k = getattr(models, args.backbone)()  # Key Encoder

        # Add the mlp head
        args.in_channels = self.encoder_q.fc.in_features
        self.encoder_q.fc = models.mlphead(args)
        self.encoder_k.fc = models.mlphead(args)
        ...
        self.register_buffer("queue", torch.randn(self.queue_size, 512))
    ...
    def shuffled_idx(self, batch_size):
        ...
        shuffled_idxs = torch.randperm(batch_size).long().to(self.args.device)

        reverse_idxs = torch.zeros(batch_size).long().to(self.args.device)

        value = torch.arange(batch_size).long().to(self.args.device)
    ...
    def InfoNCE_logits(self, f_q, f_k):
        ...
        pos = torch.mean(torch.mm(f_q, f_q.T), dim=1).unsqueeze(-1)
